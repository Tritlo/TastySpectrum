...
totalPassing, totalFailing :: Spectrum -> Integer
...
totalPassFail :: Spectrum -> (Integer, Integer)
totalPassFail (res, _, _) = (toInteger p, toInteger f)
  where
    p = length $ filter id $ map (\(_, r, _) -> r) res
    f = length res - p
...
-}
dstar :: Integer -> Spectrum -> [(Label, Double)]
...
passFailEvals ::
    -- | A source code location, including the
    Label ->
    -- | (EvalsOfPassing,EvalsOfFailing)-Tests
    ([Integer], [Integer])
