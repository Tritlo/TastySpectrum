...
leafDistanceList labels =
    map
        ( fromMaybe (error "leafDistanceList: index not found")
            . flip L.findIndex levels
            . IS.member
            . loc_index
        )
        labels
  where
    (_, apc, pdc) = genParentsAndChildren labels
    ...
    dists !sf !n = if IS.null n' then [n] else (n : dists sf' n')
      where
        n' =
            ( IM.keysSet $
                IM.filter (not . IS.disjoint n . snd) pdc
            )
                IS.\\ sf'
        sf' = sf `IS.union` n
...
rootDistances labels =
  ...
  where
    (_, _, pdc) = genParentsAndChildren labels

genParentsAndChildren ::
    [Label] ->
    ( IntMap Label -- Map of loc_index to labels
...
genParentsAndChildren labels = (imap, all_parents_and_children, parent_li_and_children)
  where
    !imap = IM.fromAscList $ map (\l@Label{loc_index = li} -> (li, l)) labels
    parents_and_children (Label{loc_pos = p, loc_index = i, loc_group = g}) =
        mapMaybe
            ( \Label{loc_pos = lp, loc_index = li, loc_group = lg} ->
                let hp = toHpcPos p
                    hlp = toHpcPos lp
                 in if g == lg && li /= i
                        then
                            if hp == hlp
                                ...
                                then Just $ if li < i then Left li else Right li
                                    ...
                                    if insideHpcPos hp hlp
                                        then Just (Left li)
                                        else
                                            if insideHpcPos hlp hp
                                                then Just (Right li)
                                                else Nothing
                        else Nothing
            )
    ...
    direct_parent_and_children li = (direct_parent, direct_children)
      where
        (ps, cs) = all_parents_and_children IM.! li
        ...
        !direct_parent =
            if IS.null ps
                then Nothing
                                ...
                                map (\pi -> (pi, fst $ all_parents_and_children IM.! pi)) $
                                    IS.toList ps
                          ) of
    ...
    !parent_and_direct_children =
        IM.fromAscList $
            map
                ( \Label{loc_index = li} ->
                    (li, direct_parent_and_children li)
                )
                labels
    !parent_li_and_children =
        IM.fromAscList
            $ map
                ( \Label{loc_index = li} ->
...
genForest labels = map (toTree . fst) roots
  where
    (imap, _, pdc) = genParentsAndChildren labels
