...
import Trace.Hpc.Mix (Mix (..), MixEntry, readMix)
...
instance IsOption SparseSpectrum where
    ...
    optionHelp =
        return $
            "Create a *non-sparse spectrum*, keeping all "
...
instance IsOption TypedSpectrum where
    ...
    optionHelp =
        return $
            "Create a *non-typed spectrum*, setting all types to []."
...
testSpectrum = TestManager
    [ Option (Proxy :: Proxy GetTestSpectrum)
            ...
            GetTestSpectrum True -> Just $ do
                ...
                spectrums <- forM tests $ \(t_name, test) -> do
                    ...
                    if all IM.null (Map.elems new_map)
                        then
                            let warn = hPutStrLn stderr
                             in ( warn $
                                    "No expression touched in the test! Make sure spawned"
                ...
                let parseTypes :: String -> IO [(String, [String])]
                    parseTypes m = do
                        ...
                        if not exists
                            then
                                error $
                                    "Types file does not exist! Please re-run with "
                ...
                let toCanonicalExpr file hpc_pos = file ++ ':' : show hpc_pos
                    ...
                    toRes (t_name, t_res, tix_maps) =
                      ...
                      where
                        -- eRes :: (String, (String, [HpcPos])) -> [Integer]
                        eRes (mod, (_, hpcs)) =
                            case tix_maps Map.!? mod of
                                Just im ->
                                    map (flip (IM.findWithDefault 0) im) $
                                        if sparseSpectrum
                                            then
                                                let t_inds = touched Map.! mod
                                                 in IS.toList $ t_inds
                                            else [0 .. (length hpcs)]
                                Nothing -> replicate (length hpcs) 0

                    header =
                        "test_name,test_type,test_result,"
...
-}
unfoldTastyTests ::
    -- | A collection of Tasty Tests
    TestTree ->
    -- | A list of (TestName, TestType, Test
...
-}
checkTastyTree ::
    -- | Timeout how long the test will be run atmost - if timeout is reached the test is errore`d.
    Timeout ->
    -- | Executable Tasty Test Tree - meant to be a single test!
    TestTree ->
    -- | Test Result. True on pass, False on fail,timeout or error
    IO Bool
